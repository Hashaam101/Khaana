    ...
    return (
      <Link href={`/store/${store.id}`} className="store-card block">
        <div className="flex bg-surface rounded-xl overflow-hidden shadow-sm border border-edge">
          {/* Image */}
          <div className="w-28 h-28 flex-shrink-0 relative overflow-hidden bg-surface-tertiary">
            <Image
              src={store.imageUrl}
              alt={store.name}
              fill
          ...
          <div className="flex-1 p-3 flex flex-col justify-between min-w-0">
            <div>
              <div className="flex items-center gap-1.5 mb-0.5">
                ...
                <h3 className="font-semibold text-sm text-content truncate">
                  {store.name}
                </h3>
              </div>
              <p className="text-xs text-content-tertiary">{store.category}</p>
            </div>
            ...
            <div className="flex items-center justify-between mt-1">
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs font-medium text-content">{store.rating}</span>
              </div>
  ...
  return (
    ...
    >
      <div className="bg-surface rounded-xl overflow-hidden shadow-sm border border-edge">
        {/* Image */}
        <div className="h-28 w-full relative overflow-hidden bg-surface-tertiary">
          <Image
            src={store.imageUrl}
            alt={store.name}
            fill
          ...
          {/* Rating badge */}
          <div className="absolute top-2 right-2 bg-surface/90 backdrop-blur-sm rounded-lg px-1.5 py-0.5 flex items-center gap-0.5 z-10">
            <Star size={10} className="star-filled fill-amber-400" />
            <span className="text-[11px] font-semibold text-content">{store.rating}</span>
          </div>
        ...
        <div className="p-3">
          ...
          <p className="text-xs text-content-tertiary mb-1.5">{store.category}</p>
          <div className="flex items-center gap-1 text-xs text-content-tertiary mb-2">
            <Clock size={11} />
            <span>
              {store.pickupDay} {store.pickupTime.split(" - ")[0]}
            </span>
